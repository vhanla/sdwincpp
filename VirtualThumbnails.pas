...
begin
  ...
  else if Ext = 'png' then Result := TPngImage;
  ...
  begin
    ...
    try
      try
        ...
        if ThumbnailExtracted and (Bitmap.AlphaFormat <> TAlphaFormat.afIgnored) then // UsesAlphaChannel(Bitmap) then {TODO: temporary fix by vhanla}
          ConvertBitmapEx(Bitmap, Bitmap, BackgroundColor);
        {$ENDIF}
      except
...
begin
  ...
  with FValidImageFormats do begin
    CommaText := '.jpg, .jpeg, .jpe, .jif, .bmp, .emf, .wmf, .png';
    {$IFDEF USEIMAGEEN}
